...
Quants <- df_protein_mcap %>%
  group_by(Treatment, hpf, Fraction) %>%
  summarize(quant25 = quantile(df_protein_mcap, probs = q[1]),
            quant75 = quantile(df_protein_mcap, probs = q[2]),
            IQRbyGroup=IQR(df_protein_mcap))
```
...
```{r}
model<-aov(Concentration~Treatment*hpf, data=df_protein)
summary(model)
